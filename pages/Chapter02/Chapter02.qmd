...
The probability of a coin coming out $n_H$ heads after $N$ flips follows the binomial distribution, what we do not know is the true rate $f_H$ (how much and in what direction the coin is bent). 


$$
L(n_H+1=Heads|N,n_H) = \int_{f_H=0}^{1}{df_H L(f_H|N,n_H)}
$${#eq-lk-rates}

We can use the binomial to get the likelihood of different rate hypothesis.

$$
L(f_H|N,n_H) = P(n_H|N,f_H)P(f_H)
$$

And substitute in @eq-lk-rates.

$$
L(n_H+1=Heads|N,n_H) = \int_{f_H=0}^{1}{df_H P(n_H|N,f_H)P(f_H)}
$$

Since we are assuming a uniform prior on $f_H$ 

$$
\int_{f_H=0}^{1}{df_HP(f_H)} := 1
$$

Which leaves

$$
L(n_H+1=Heads|N,n_H) = \int_{f_H=0}^{1}{df_H P(n_H|N,f_H)}
$$
